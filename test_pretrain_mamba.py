...
from recformer import RecmambaForPretraining, RecmambaConfig, LitWrapper, Test_LitWrapper
...
parser.add_argument("--train_file", type=str, default="pretrain_data/train.json")
parser.add_argument("--dev_file", type=str, default="pretrain_data/dev.json")
parser.add_argument("--item_attr_file", type=str, default="pretrain_data/meta_data.json")
parser.add_argument("--output_dir", type=str, default="result/recmamba_pretraining")
parser.add_argument("--num_train_epochs", type=int, default=32)
...
parser.add_argument(
    "--hydra_ckpt", type=str, default="hydra_ckpt/bert-base-uncased.bin"
)
parser.add_argument("--fix_word_embedding", action="store_true", default=True)


tokenizer_glb: RecmambaTokenizer = None
...
def main():
    ...
    config = RecmambaConfig.from_pretrained(
        args.model_name_or_path, **hydra_config_params
    )
    ...
    pytorch_model = RecmambaForPretraining(config)
    pytorch_model.load_state_dict(torch.load(args.hydra_ckpt))

    if args.fix_word_embedding:
        print("Fix word embeddings.")
        for param in pytorch_model.bert.embeddings.word_embeddings.parameters():
            param.requires_grad = False

    model = Test_LitWrapper(pytorch_model, learning_rate=args.learning_rate)
