...
from hydra.bert.src.bert_layers import BertModel, BertEncoder
...
class RecformerEmbeddings(nn.Module):
    ...
    def __init__(self, config: RecformerConfig):
        ...
        self.token_type_embeddings = nn.Embedding(
            config.token_type_size, config.hidden_size
        )
        self.item_position_embeddings = nn.Embedding(
            config.max_item_embeddings, config.hidden_size
        )
...
class RecformerPooler(nn.Module):
    ...
    def forward(
        self, attention_mask: torch.Tensor, hidden_states: torch.Tensor
    ) -> torch.Tensor:
        ...
        if self.pooler_type == "cls":
            ...
            output = (hidden_states * attention_mask.unsqueeze(-1)).sum(
                1
...
class RecformerModel(LongformerPreTrainedModel):
    ...
    def _pad_to_window_size(
        ...
        if padding_len > 0:
            ...
            if item_position_ids is not None:
                item_position_ids = nn.functional.pad(
                    item_position_ids, (0, padding_len), value=pad_token_id
                )
    ...
    def forward(
        ...
        pooled_output = (
            self.pooler(attention_mask, sequence_output)
...
class RecformerForPretraining(LongformerPreTrainedModel):
    ...
    def forward(
        ...
        if mlm_outputs_a is not None and mlm_labels_a is not None:
            ...
            masked_lm_loss_a = loss_fct(
                prediction_scores_a.view(-1, self.config.vocab_size),
                mlm_labels_a.view(-1),
            )
        ...
        if mlm_outputs_b is not None and mlm_labels_b is not None:
            ...
            masked_lm_loss_b = loss_fct(
                prediction_scores_b.view(-1, self.config.vocab_size),
                mlm_labels_b.view(-1),
            )
        ...
        if self.config.finetune_negative_sample_size <= 0:  ## using full softmax
            ...
            candidates = torch.cat(
                (
                    ...
                    torch.randint(
                        0,
                        self.config.item_num,
                        size=(batch_size, self.config.finetune_negative_sample_size),
                    ).to(labels.device),
                ),
                dim=-1,
            )
...
class RecmambaConfig(BertConfig):
    ...
    def __init__(
        ...
        ###
        use_position_embeddings: bool = True,  # Recformer에서는 항상 True
...
class RecmambaModel(BertPreTrainedModel):
    def __init__(self, config: RecmambaConfig):
        super().__init__(config)
        self.config = config
        ...
        self.embeddings = RecmambaEmbeddings(config)
    ...
    def forward(
        ...
        # Hydra BertEncoder interface: forward(hidden_states, attention_mask, output_all_encoded_layers, subset_mask)
        encoder_outputs = self.encoder(
            embedding_output,
            attention_mask,
            ...
            subset_mask=None,  # For efficient training, could be calculated by masked_tokens_mask
        )
...
class RecmambaForPretraining(BertPreTrainedModel):
    def __init__(self, config: RecmambaConfig):
        ...
        self.bert = RecmambaModel(config)
...
class RecmambaEmbeddings(nn.Module):
    """
    Same as BertEmbeddings with a tiny tweak for positional embeddings indexing.
    """
...
if __name__ == "__main__":
    config = RecmambaConfig()
    model = RecmambaModel(config)
